...
- Both the advertiser SDK and the user's wallet create the same attribution_hash using their respective DIDs, the campaign ID, product, day, hour, type of device, ad display characteristics (e.g. size), creative type, click zone, the nonce provided by the advertiser, can all be linked to the attribution_hash and any other values requested by the advertiser. *Note* - the attribution hash must both 1) be easily identifiably by advertisers 2) change for every impression lest it be used to correlated the user and track them
